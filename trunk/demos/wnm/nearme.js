...
// Search for businesses the selected vertical and display them in a list
...
function add_current_location_to_map(){
		...
		var image = new google.maps.MarkerImage('img/blue_dot_circle.png',
...
function location_query(lat, lon, radius, vertical,
    ...
    if (api_cache[qid]) { // If the same query has already been requested
        ...
        url += "?q=" + JSON.stringify(q) + "&apikey=demo&jsoncallback=?"
        $.getJSON(url, /*{q: $.toJSON(q), apikey: "demo", jsoncallback: "?"},*/ function(resp) {
