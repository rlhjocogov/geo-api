  ...
  var updateContainer,
      updateHTML = "<li class='hentry status u-USERNAME' id='status_ID'>\
        ...
        <span class='status-body'>\
          <img alt='Icon_lock' class='lock' src='http://assets2.twitter.com/images/icon_lock.gif' title='REAL_NAME‚Äö√Ñ√¥s updates are protected‚Äö√Ñ√Æ please don‚Äö√Ñ√¥t share!' />\
          <strong><a class='screen-name' href='USERNAME' title='REAL_NAME'>USERNAME</a></strong>\
          <span class='entry-content'>TEXT</span>\
          <span class='meta entry-meta'>\
            <a class='entry-date' href='/USERNAME/status/ID' rel='bookmark'>\
              <span class='published' title='CREATED_AT'>CREATED_AGO</span>\
            </a>\
        ...
        <span class='actions'><div>\
          <a class='fav-action FAV_CLASS' id='status_star_ID' title='FAV_ACTION this update'>&nbsp;&nbsp;</a>\
          <a class='del' title='delete this update'>&nbsp;&nbsp;</a>\
          <a class='reply' href='/home?status=@USERNAME%20&amp;in_reply_to_status_id=ID&amp;in_reply_to=USERNAME' title='reply to USERNAME'>&nbsp;&nbsp;</a>\
        </div></span>\
      </li>"
...
if (address && /[+-]?\d+\.\d+,[+-]?\d+\.\d+/.test(address.textContent)) {
  ...
  address.innerHTML += '<br><a class="googlemap" target="_blank" href="http://maps.google.com/maps?q=' + coordinates + '"><img src="http://maps.google.com/staticmap?center=' + coordinates + '&markers=' + coordinates + ',red&zoom=13&size=165x165&key=' + API_KEY + '" alt=""></a>'
...
if ($et.sidebar) {
  ...
  scriptInfo.appendChild(document.createTextNode(' ' ))
...
if (wrapper) checkUserscriptUpdate(scriptURL, $et.scriptSize, function() {
  var notice = $E('span', { id: 'userscript_update' },
    'Better Twitter (user script) has updates (you have v' + scriptVersion + '). ')
...
#timeline .status.last-read { background: #f2f2f2; }\
...
body.mini #side_base { -moz-border-radius: 0 !important; -webkit-border-radius: 0 !important; border-left: none !important; display: block; position: absolute; left: 0; top: 0; height: 40px; width: 423px; padding-left: 140px; }\
...
body.mini #side #primary_nav ~ *, body.mini #side #message_count, body.mini #side .about, body.mini #side .promotion { display: none; }\
...
retweet = {
  ...
  initialize: function() {
    ...
    if (retweet.status.pageuser == retweet.status.user) {
     retweet.status.pageuser = null;
    }    

    retweet.addstyle([
      'textarea#status { height: 3.5em; }',
      'ol#timeline li span.actions a { padding:1px 8px; }',
      'ol#timeline li span.actions a.retweet,',
      'div#permalink div.actions a.retweet { width:20px; height:16px; background:url(data:image/png;base64,'+
              'iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAAXNSR0IArs4c6QAAAAZiS0dEAP8A/wD/oL2nkwAAAAlw'+
              'SFlzAAALEwAACxMBAJqcGAAAAAd0SU1FB9kIFwAmHj2KfecAAAAIdEVYdENvbW1lbnQA9syWvwAAAXlJREFUOMul082L'+
              'z1EUBvDPb7ws1IgxO9loTGMjkaQ8K0v+BsnCclBS2FBjIbNQsrBgQxqpWVmw1CmRbIRSSqMsvA5RGnnb3KnvTD9Jbt26'+
              '5zn3PPec557Df67enxxV1cOaDvQjyexfCapqJ67iCx5gX8d9NMnZPxJU1XI8wUiDprEN65r9GXvwLMlrGFiUwKFOMNzC'+
              'sY49iIn54H4Eg/jezg9xCVOYadhPHFxQQlUtxXCzlzThJnEyyd2qGsPm5n+HN9iPw0l+9VrtEzjRLo0nOd9H3JU4gwMt'+
              '871JrvTad91vYsFHjCZ5u4hgCM+xukGvMDKAHZ1guIYtizNI8gGnOtBabOpV1TI8xiheYizJ10WvD7e0Z/EIY3iKrfMa'+
              '7MZNnMaqTuydJDeq6iJW4BOGWhnnktzudV6ZbGrv6hBcwOXWkd0vf4GNSea64HFs6DMWo3365UiSuQWNlOQbxlttM22/'+
              'x3Xca6rPYCrJ9D+NbFVtr6r1/Xy/AQEbc4im7PyoAAAAAElFTkSuQmCC) no-repeat center; }'
    ]);
    
    if(retweet.status.currentpage !== 'show') {
      $('ol#timeline li.status').livequery(function() {
        if ($('body').attr('id') != 'search') {
          var parentelement = $(this).children('span.actions').children('div');
        } else {
          var parentelement = $(this).children('span.actions');
        }
        parentelement.append('<a title="retweet" class="retweet" rel="'+$(this).attr('id')+'></a>');
      });
      if (!retweet.status.pageuser) {
        $('ol#timeline a.retweet').livequery('click', function() {
          retweet.updatestatus($(this).attr('rel'));
          return false;
        });
      } else {
        $('ol#timeline a.retweet').livequery(function() {
          $(this).attr('href', '/home?status='+retweet.util.encodeurl(retweet.retweetstatus($(this).attr('rel'))));
        });
      }
    } else {
      $('div#permalink div.actions a.reply').after('<a title="retweet" class="retweet"></a>');
      $('div#permalink div.actions a.retweet').attr('href', '/home?status='+
  ...
  retweetstatus: function(statusid) {
    ...
    if (!username) {
      username = (currentpage != 'search') ? $('li#'+statusid+' a.screen-name').html() : $('span#msgtxt'+statusid.substr(7)).siblings('a').html();
    } 
    var statustext = (currentpage != 'search') ? $('li#'+statusid+' span.entry-content').html() : $('span#msgtxt'+statusid.substr(7)).html();
  ...
  update:function() {
    if (tweetpreview.statustext.length) {
      ...
      var previewhtml = tweetpreview.statustext.substr(0,160)
                                    ...
                                    .replace(/(^|\s|[\?\.,;:\!\-]+)#([^#\?\./,;:`‹›«»~!\[\]\(\)%\+\-\}\{\s]+)/g, tweetpreview.format_hashtag_links)
                                    .replace(/<a[^>]+>[^<]+<\/a>/gmi, tweetpreview.format_cut_links) 
